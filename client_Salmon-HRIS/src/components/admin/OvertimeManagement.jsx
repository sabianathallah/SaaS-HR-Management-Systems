      ...
      >
        <div className="space-y-4">
              ...
              <div className="text-sm text-gray-700 space-y-1">
                ...
                {selectedRequest.status === 'approved' && (
                  <p className="text-orange-600 font-medium mt-2">
                    ⚠️ This will change the status from APPROVED to REJECTED
                  </p>
                )}
              </div>
            </div>
          ...
          {rejectReason.trim().length > 0 && rejectReason.trim().length < 10 && (
            <p className="text-xs text-red-500">⚠️ Rejection reason must be at least 10 characters</p>
          )}
        </div>
      </Modal>
          ...
          {editStatus === 'rejected' && (
            <div>
              ...
              {editReason.trim().length > 0 && editReason.trim().length < 10 && (
                <p className="text-xs text-red-500 mt-1">⚠️ Must be at least 10 characters</p>
              )}
            </div>
          )}
